...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['image'])) {
    ...
    $curl = curl_init();
    curl_setopt($curl, CURLOPT_URL, "http://127.0.0.1:5000/predict/pest");
    ...
    $prediction_result = $result['prediction'] ?? "Please upload a clearer, well-lit image, or check if there might be an issue with the Flask Server.";
...
<head>
  ...
  <title>Peanut Pest Detection</title>
...
<body>
  ...
  <section class="peanut-assessment">
    <div class="container padding-top-custom">
      <div class="content">
        <h1>Peanut Pest Detection</h1>
        <p>Identify pest infestations swiftly with advanced AI-based image analysis tailored for peanut crops. The system detects visual cues of pest presence, such as bite marks, larvae, and external damage, and can identify pests like Aphids, Armyworms, Caterpillars, Thrips, and Wireworms. This enables a quick response before the problem spreads. Minimize crop damage, enhance field management, and protect your yield with accurate, real-time pest monitoring.</p>
        <form action="image_pest_detection.php" method="post" enctype="multipart/form-data">
          <div class="file-input-wrapper">
            <input type="file" class="choose-image" name="image" required accept="image/*">
          </div>
          
          <button type="submit" class="btn assessment-btn">Analyze Pest Image</button>
        </form>
      </div>
      <div class="peanut-image">
        <img src="images/peanut-pest-detection.png" alt="Peanut quality inspection">
      </div>
    </div>
  </section>
  ...
  <?php if (!empty($prediction_result)) : ?>
    <div class="container">
        <div class="jumbotron text-center py-4 px-4 mt-4">
          <h1 class="display-5 mb-4"><i class="fas fa-search fa-xs"></i> Pest Detection </h1>

          <?php echo isset($result['prediction']) 
            ? '<p class="lead">The peanut pest falls under the category of:</p>' 
          ...
          <!-- <p><strong>Model Used:</strong> ConvNeXt-Tiny</p>
          <p><strong>Accuracy: </strong> 91.94%</p>
          <p><strong>Precison:</strong> 88.27%</p>
          <p><strong>Recall:</strong> 87.83%</p>
          <p><strong>F1 Score:</strong> 87.93%</p> -->


          <?php if (isset($result['prediction']) && !empty($result['prediction']) && $result['prediction'] != 'Unknown'): ?>
            <img src="images/Peanut_Pest_Detection_Graph.png" alt="Statistical Graph" class="img-fluid">
          <?php endif; ?>
